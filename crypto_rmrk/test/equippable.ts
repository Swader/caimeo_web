...
describe('SoulShard Assets', async () => {
  ...
  describe('Init', async function () {
    it.skip('can get names and symbols', async function () {
      expect(await equip.name()).to.equal('SoulShard');
      expect(await equip.symbol()).to.equal('SSHARD');
    });
  });
});
